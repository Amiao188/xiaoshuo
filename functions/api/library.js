...
export async function onRequestGet({ env }) {
  const rows = (await env.DB.prepare("SELECT id, title, author, tag, publish_date, content_json FROM stories WHERE status = 'published' ORDER BY publish_date DESC, created_at DESC").all()).results || [];
  const grouped = {};
